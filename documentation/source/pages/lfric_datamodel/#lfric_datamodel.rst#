...
documents will periodically be updated at the following location in the
Met Office Shared Repository System. An account and password is required
if external to the Met Office:
...
This document provides an overview of the LFRic Infrastructure, and how
to use it to write code for earth system modelling applications. The
LFRic Infrastructure comprises Fortran code that supports scientific
applications written according to a software architecture called
**PSyKAl** (see Section `[sec:concepts] <#sec:concepts>`__ for a
description). Some of the application's Fortran code is generated by a
separate program called **PSyclone**. PSyclone generates the
platform-specific code required to support the parallel deployment and
optimisation of the application. This document includes an overview of
PSyclone including a simple worked example. PSyclone has its own
comprehensive documentation :raw-latex:`\cite{psyclone}`.
...
model will be based on the GungHo dynamical core
:raw-latex:`\cite{gunghocs}` coupled to subgrid parametrizations, many
of which are also used within the Unified Model (UM). The new model is
commonly referred to as the **LFRic Atmosphere Model**.
...
of the GungHo :raw-latex:`\cite{gunghocs}` dynamical core and the more
traditional finite difference schemes of the UM physics codes. GungHo
has been developed to run on meshes such as the cubed sphere or diamond
sphere. Therefore the LFRic Infrastructure needs to support a range of
generalised unstructured meshes.
...
system. These systems are not documented here -- refer to the LFRic Trac
wiki system.
...
installation from head of the LFRic trunk. The document aims to identify
some of the areas which are likely to change due to ongoing or
...
as deep into the technical details as is required to describe the basics
of the LFRic data model, the architecture and the make up of an
LFRic-based scientific application.

Section `[sec:concepts] <#sec:concepts>`__ gives an overview of the
novel requirements of the LFRic model that are driven by its support for
the GungHo finite element formulation and the unstructured mesh, and
defines some terms. Section `[sec:psykal] <#sec:psykal>`__ describes the
PSyKAl architecture of the core science code within the model, and
illustrates how scientists will typically contribute new science to
existing models. Having explained the high level architecture,
Section `[sec:miniapps] <#sec:miniapps>`__ moves on to describes the the
broad structure of a typical LFRic-based application. Code examples
derived from LFRic atmosphere model illustrate the descriptions.

Further technical documentation is under development which will provide
comprehensive descriptions of all of these parts of the system
:raw-latex:`\cite{lfric_wikidocs}`.

Key Concepts and Requirements[sec:concepts]
===========================================
...
The PSy layer code breaks the global data down into chunks and passes
the chunks to the kernels, applying any requested parallel computation
strategies.
...
PSyclone can generate code that includes directives and calls to support
distributed memory parallel operations (MPI) such as halo swaps, and
shared-memory parallelism such as OpenMP parallel loops. Algorithms can
be written to group several kernel calls together: by examining metadata
in such a group of kernels PSyclone can assess dependencies between
kernels and generate correct code that takes these dependencies into
account. For example, it may be optimal to compute one kernel deeper
into the halo so as to remove a halo swap that would otherwise be
required to run the next kernel.
...
-  To allow for concurrent communication and computation, support for
   ...
   detail in a separate LFRic document :raw-latex:`\cite{distmem_impl}`.
...
``r_def`` and ``i_def``. Within the code base will be seen declarations
...
and vertices) that enclose the volume. Neigbouring cells can share the
...
function associated with the dof is unity, and all the other basis
functions are zero. Therefore, the value of the field at the location of
the dof is the value of the dof itself.
...
.. figure:: images/basisOrder2.svg
   ...
   The first figure shows a set of basis functions (labelled A, B
...
.. figure:: images/basisOrder2_dofs.svg
   ...
   This second figure shows the nominal location of three degrees of freedom
   ...
   total displayed by the follosing figure.

.. figure:: images/basisOrder2_field.svg

   The final figure represents the value of the field within the whole
...
In Figure b), however, the dofs are not shared between cells. While they
...
at the cells' boundary can have different values to each other, it
...
.. figure:: images/CG2_2D.svg
   ...
   A continuous and a discontinuous one-dimensional field, showing
   ...
   cells, and so associated with the edge between the two cells. The
   dofs at the cell edges for the discontinuous field are at the same
   real location, but they are not shared with the neighbouring cell,
   and so are associated with the cell volume rather than its edge. In
...
such as a lat-lon mesh it may be possible to compute the location of all
dofs belonging to a given cell based on the cell location. For a general
unstructured mesh, though, this is not feasible. Instead, for each type
of field, look-up tables are created that can be used to reference all
dofs in each cell. These look-up tables are referred to as **dof-maps**.

As noted in Section `[sec:concepts] <#sec:concepts>`__ LFRic stores
dof-maps for the lowest cell of the 3D mesh as the dof-maps for the
cells in the next layer can be computed by incrementing the dof-map
addresses. The ability to infer the dofmaps of successive cells in a
vertical column enables kernels to be written that operate on a column
of cells based on the dof-map for just one cell.
...
of the lines in the two dof-maps, and so can operate on the bottom cell
and the column of data above it. As discussed in
Section `[sec:concepts] <#sec:concepts>`__ the kernel is called via the
PSy layer code which calls the kernel many times with different chunks
of the global data, and for a kernel, the appropriate chunk is all the
data in a vertical column of cells.

A simplified kernel and PSy layer example[sec:simplePsy]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Section `[sec:psykal] <#sec:psykal>`__ describes the PSyKAl
implementation in much more detail using code snippets from the current
...
The sharing of dofs means that the kernel must be performing some sort
...
would over-write the results of the first. In fact, PSyclone includes
rules which will reject a kernel that seeks to write data to a
continuous field as opposed to incrementing data it already contains.

LFRic Function spaces and element order[sec:function_spaces_intro]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

This section introduces the LFRic function space object which underpins
fields used in LFRic. In previous sections it was noted that the
function space is a concept in the finite element method whereby data
and basis functions can define a field that spacially varies within an
individual cell, and this was illustrated for individual cells in
Section `[sec:dofs] <#sec:dofs>`__. The implementation of the function
space object in LFRic goes further than this as it maps all of the data
to the whole three-dimensional mesh.

LFRic supports a range of **function space types**. The function space
type is defined by the type of basis functions which considers both the
functions and the locations of the dofs. Each of the several different
function space types has particular layout of dofs within a cell. For
example, the function space with face centred dofs is referred to as
:math:`\mathbb{W}_{2}` and the cell-centred function space as
:math:`\mathbb{W}_{3}`. Essentially, the number represents the number of
dimensions of the entity on which the dof is placed. Following this
pattern, then, the :math:`\mathbb{W}_{0}` function space has dofs on the
vertices and the :math:`\mathbb{W}_{1}` function space has dofs on edges
of the mesh.

This is not the full story. The above descriptions are for the "lowest
order" versions of the four function spaces. At lowest element order,
the basis functions are a constant function (for :math:`\mathbb{W}_{3}`)
or linear functions. Higher order function spaces are possible for each
type of function space. Each higher order function space has more basis
functions and therefore more dofs. The basis functions are changed from
being constant to linear for :math:`\mathbb{W}_{3}`, and from linear to
quadratic for the other function spaces.
...
.. figure:: images/corner_W1.svg
   ...
   Details of the locations of dofs for the spaces :math:`\mathbb{W}_1`
   and :math:`\mathbb{W}_2, k = 1`. The faint boxes and lines are, in
...
neighbouring cells but discontinuous across the boundary with vertical
neighbours.

Overview of the PSyKAl design[sec:psykal]
=========================================

This section provides a brief walkthrough of the PSyKAl design. It uses
code snippets from the current implementation of the GungHo dynamics.
There are similarities between these code snippets and those used in
Section `[sec:simplePsy] <#sec:simplePsy>`__. Additional details shown
here will reference other key aspects of the LFRic infrastructure and
the capabilities of PSyclone :raw-latex:`\cite{psyclone}`.
...
fields. As noted previously, numerical operations on model fields within
algorithms are operations on the whole field and not on subsets of the
field. Operations on fields are represented as calls to a non-existent
subroutine called ``invoke``. The ``invoke`` subroutine call can have
one or more argument. Each argument is either a kernel type as written
by a scientist or software engineer, or an arithmetic or linear algebra
operation which has support built into the PSyclone API (see the
Dynamo0.3 API description in the PSyclone documentation
:raw-latex:`\cite{psyclone}` for a list of so-called **built-in**
operations). Each kernel or built-in operation takes a set of arguments.
...
used to name the subroutine that PSyclone generates. Using the ``name``
argument can be helpful when debugging and profiling the code as it
makes it easier to find the relevant PSy layer subroutine. By default,
PSyclone will name the subroutine ``invoke_`` followed by a number.
...
subset will be a single vertical column of data, which is the first
target for LFRic implementation.
...
Note, that while the metadata code is correct Fortran, its only purpose
...
too important. The main aim is to show in the next section how this
...
run: a :math:`\mathbb{W}_{3}` field is a cell-centred field typically
used for mass quantities; a :math:`\mathbb{W}_{2}` field has dofs on the
cell faces and is used for vector quantities such as fluxes and
velocities. A :math:`\mathbb{W}_{chi}` field is a coordinate field
representing the fixed coordinates of the mesh.

The subroutine interface for the kernel is as follows. Clearly it has
many more arguments than the original ``invoke`` call, and each argument
is described in Section `[sec:psy_example] <#sec:psy_example>`__ below.
The important thing to note is that all of the arguments and the order
of the arguments derive entirely from the kernel metadata. In fact, the
PSyclone toolset includes a stub generator ``genkernelstub`` which will
generate the following subroutine call and all the argument declarations
based on the above metadata. See the PSyclone documentation for details.
...
kernels directory of the GungHo source code, so only a brief description
...
This section provides a brief overview of the general operation and use
of PSyclone, before describing in more detail the PSy layer code that
could be generated in support of the ``invoke`` call and kernel metadata
described in the preceding sections. As noted elsewhere, PSyclone has
its own comprehensive documentation :raw-latex:`\cite{psyclone}`.

As noted previously, PSyclone supports several different APIs including
...
It is important to note here that PSyclone should be regarded as a tool
...
PSy layer code generation example[sec:psy_example]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
``undf_chi, undf_w2, undf_w3``
   ...
   function space. In the example code in
   Section `[sec:dofs] <#sec:dofs>`__ we used the more meaningful
   variable name ``total_dofs``. The ``u`` stands for "unique": where
...
snippets, the kernel call line has been re-formatted for readability).
The final set of information from the proxy is the actual field data
which are referenced directly from the proxy.
...
The loop range for this kernel call runs over the local domain excluding
any halo cells. This means that after the loop has completed, the halo
region of the output field is now out of date. PSyclone signals this by
marking the halo as "dirty":
...
The last part of the ``invoke`` introduced on Page  includes a summation
...
Building LFRic applications[sec:miniapps]
=========================================
...
Fields and field collections[sec:fields_intro]
----------------------------------------------

Two of the main data structures used by a scientist writing LFRic models
and algorithms are fields and field collections. As has been discussed
in Section `[sec:function_spaces_intro] <#sec:function_spaces_intro>`__,
...
created then the field initialised.
...
described in Section `[sec:psykal] <#sec:psykal>`__.

Field collections and the Depository[sec:field_collections_intro]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
UGRID files using XIOS :raw-latex:`\cite{xios}`. XIOS uses an input
