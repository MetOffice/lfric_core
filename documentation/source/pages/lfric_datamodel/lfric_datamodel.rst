...
**PSyKAl** (see `here <section concepts>` for a
...
The LFRic Infrastructure underpins the development of the new Momentum
Atmosphere model being developed to replace the Met Office Unified
Model. The new model will be based on the GungHo dynamical core
coupled to subgrid parametrizations, many of which are also used
within the Unified Model (UM). The Momentum model is often referred to
as the **LFRic Atmosphere Model**.
...
The :ref:`Key Concepts and Requirements
<section concepts>` section  gives an overview of the
...
defines some terms. An :ref:`overview of the PSyKAl design <section
psykal>` follows, describing the
PSyKAl architecture of the core science code within the model, and
illustrating how scientists will typically contribute new science to
existing models. Having explained the high level architecture, the
document describes the broad structure of a :ref:`typical LFRic-based
application <section apps>`. Code examples
...
Figure `2 <#fig:Mesh>`__ is given :ref:`in a later section <section dofs>`.
...
As noted :ref:`earlier <section concepts>` LFRic stores
...
implementation of the PSyKAl architecture is shown for a kernel
operating on fields on the two function spaces whose dof locations are
represented in Figure `2 <#fig:Mesh>`__: a field with data on faces,
``face_data``, and a field with data within the cell volume,
``volume_data``. Among other arguments, the kernel subroutine
...
individual cell, and this was illustrated for individual cells
:ref:`above <section dofs>`. The implementation of the function
...
:ref:`the first simplified examples <section simplePsy>`. Additional details shown
here will reference other key aspects of the LFRic infrastructure and
the capabilities of PSyclone.
...
is described :ref:`below <section psy example>`.
...
PSyclone has its own `documentation
...
``ndf_chi, ndf_w2, ndf_w3``
   ...
   Figure `16 <#fig:k0k1w0-w3>`__ e). In previous :ref:`example code
   <section simplePsy>` we used the more meaningful
   variable name ``dofs_per_cell``.

``undf_chi, undf_w2, undf_w3``
   are the dimensions of the whole field data array for fields on each
   function space. In the :ref:`example code
   <section simplePsy>` we used the more meaningful
...
:ref:`above <section function space intro>`,
