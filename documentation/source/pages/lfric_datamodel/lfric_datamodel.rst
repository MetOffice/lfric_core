...
software architecture called **PSyKAl** (see :ref:`here <section
concepts>` for a description). Some of the application's Fortran code
is generated by a separate program called **PSyclone**. PSyclone
...
faces. The connectivity will be represented by mappings between, for
...
In the left-hand diagram of the :ref:`above figure
<w2_w3_vert_3d>`, each cell has 6
...
represented in :ref:`the figure above <w2_w3_vert_3d>`: a field with data on faces,
...
.. figure:: images/k1_W1_dofs_circ.svg
   ...
   Locations of dofs for spaces :math:`\mathbb{W}_1` at next lowest
   in right. The image is not precise enough to show the subtleties
   of continuity as represented by the nominal dof location --
   the :ref:`figure below <corner_w1>` provides an expanded view of
   the corner dofs in the circle.
...
The kernel API and illustrative PSy layer code were introduced
:ref:`above <section simplePsy>`. This section briefly describes the
implementation of the full PSyKAl design using real code snippets
taken from the GungHo dynamics implementation. Additional details
shown here will reference other key aspects of the LFRic
infrastructure and the capabilities of PSyclone.
...
PSyclone will name the subroutine ``invoke_`` followed by a number or
by the name of the kernel if there is only one kernel listed in the
``invoke`` call.
...
``ndf_chi, ndf_w2, ndf_w3``
   ...
   :math:`\mathbb{W}_{2}` field. In previous :ref:`example code
