...
.. _section introduction to using psyclone:

Use of PSyclone for LFRic applications
======================================


This section of the documentation provides an overview describing how
the science in LFRic models should be developed. The LFRic
Infrastructure is designed to support scientific applications written
according to a software architecture called **PSyKAl**. Some of an
LFRic application's Fortran code is generated by a separate program
called **PSyclone**. PSyclone generates the platform-specific code
required to support the parallel deployment and optimisation of the
LFRic application. An overview of PSyclone, including a simple worked
example, is included below. For more detail, see the comprehensive
...
uses a mixed finite element scheme. The LFRic Infrastructure supports
both the mixed finite element scheme of the GungHo dynamical core and
the more traditional finite difference schemes of many physics
codes. GungHo has been developed to run on meshes such as the cubed
sphere or diamond sphere. Therefore the LFRic Infrastructure needs to
support a range of generalised unstructured meshes.

This document aims to introduce a new reader to LFRic with an overview
of the PSyKAl design and how scientific code will be written to follow
it. The :ref:`Key Concepts and Requirements <section concepts>`
section lists the key requirements of the LFRic model that motivate
the design, and defines some terms. An :ref:`overview of the PSyKAl
design <section psykal>` follows, describing the PSyKAl architecture
of the core science code within the model, and illustrating how
scientists will typically contribute new science to existing
models. Code examples derived from LFRic
atmosphere model illustrate the descriptions.
