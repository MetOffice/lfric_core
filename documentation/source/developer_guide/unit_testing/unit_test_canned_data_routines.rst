...
.. dropdown:: ``get_unit_test_sizes_mod.f90``
  ...
  * ``get_{function_space_name}_m3x3_q3x3x3_size`` :raw-html:`<br />`
    ...
    sizes for a field on a w0 function space using a 3x3 mesh and a 3x3x3
    quadrature: :raw-html:`<br />`
...
.. dropdown:: ``get_unit_test_dofmap_mod.f90``
  ...
  * ``get_{function_space_name}_m3x3_dofmap`` :raw-html:`<br />`
    ...
    (``map_w2(:,:)``) for a field on a w2 function space on a 3x3 mesh.
...
.. dropdown:: ``get_unit_test_entity_map_mod.f90``
  ...
  * ``get_{function_space_name}_{order}_entity_map(entity_map)``
    ...
    ``get_w2_o1_entity_map(entity_map)`` gets the list for the `W2` function
    space for element order 1. See the routine comments for the entity numbering
    which is derived from the reference cell.
...
.. dropdown:: ``get_unit_3x3x3_chi_mod.f90``
  ...
  the W0 and WChi coordinate spaces. These routines utilise dofmaps generated by
  the ``get_unit_test_dofmap_mod.f90`` module.

  * ``get_w0_3x3x3_field`` :raw-html:`<br />`
    returns a set of three coordinate fields (chi1, chi2, chi3) for the lowest
    order, W0 space on a planar domain.
  * ``get_wchi_3x3x3_field`` :raw-html:`<br />`
    returns a set of three coordinate fields (chi1, chi2, chi3) for the W_Chi
    (W3, k=1) space on a planar domain.
  * ``get_wchi_3x3x3_latlon_field`` :raw-html:`<br />`
    returns a set of three coordinate fields (chi1, chi2, chi3) for the W_Chi
    (W3, k=1) space on a portion of a spherical domain with lat-lon variation.
...
.. dropdown:: ``get_unit_m3x3_cma_data_mod.f90``
  ...
  * ``get_wtheta_w3_m3x3_cma_data`` :raw-html:`<br />`
    Computes the CMA scalars, allocates and populates the maps for a
    Wtheta-W3 CMA operator.
  * ``get_w3_w3_m3x3_cma_data`` :raw-html:`<br />`
    Computes the CMA scalars, allocates and populates the maps for a W3-W3 CMA
    operator.
  * ``get_w2v_w2v_m3x3_cma_data`` :raw-html:`<br />`
    Computes the CMA scalars, allocates and populates the maps for a W2v-W2v CMA
    operator.
