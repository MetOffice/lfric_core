...
functionality that have a clearly specified API. This will typically be
...
Test code can be identified by the ".pf" extension and are normal Fortran files
garnished with "@" (at symbol) prefixed directives. The processor substitutes
the directives with additional Fortran code and Fortran preprocessor directives.
...
procedures are simple arrays of primitive types. In a model run these arrays are
derived from infrastructure objects such as fields and function spaces. It is
very easy to rely on these infrastructure objects in the unit tests. However,
...
It is also worth testing a few expected failure modes. If the unit opens a file,
test it with a missing file. If it requires a positive integer, give it a
negative one.
...
You are about to be introduced to a number of different approaches and
...
Note the pFUnit directives prefixed with the "@" (at) symbol. These are used to
mark out the test cases with ``@test``. They are also used to denote
...
Failure to do this will lead to the tests failing to pass unexpectedly and
unpredictably as the uninitialised (or initialised elsewhere) parameters change
value.
...
As an optimisation our configuration loading procedures read the namelist on one
process and use of MPI to broadcast them to the other processes. The downside to
this is that any test which has to feign configuration must be run as an MPI
test. See below for details on MPI tests.

Bear in mind that you must feign everything the code being tested will use. If
the unit calls down to helper procedures any configuration they make use of must
also be feigned.
...
necessary not only for testing parallel code but also any test which makes use
of feign procedures.

It is easy to exploit requiring only to specify the number of processes to use
for each test.
...
In the case of MPI tests the parameterisation is handled for you. However it can
often be useful to implement your own tests in this way. In particular where you
have a series of tests where the logic is identical but the input and output
vary.
...
An example of how the above helper routines should be used can be found at:
wiki:LFRicTechnical/Testing/UnitTestKernelExample
...
If a test suffers and error (rather than a failure) then pFUnit does not always
...
For further suggestions on dealing with knotty testing problems see the page
"[Testing/WeirdUnitTest Help! The Unit Tests Have Gone Weird!]".
...
When the unit tests are run as part of the !Rose/Cylc test suites this option is
...
between its supervising process and the tests it is running. Therefore injecting
unexpected data into this stream can trip the framework up.
