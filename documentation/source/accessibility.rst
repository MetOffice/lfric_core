...
- The use of layout tables is due to the use of the `sphinx.ext.autosummary`_
  and `sphinx.ext.autodoc`_ extensions. The API documentation that these
  extensions generate can produce titles that fail `WCAG 2.2 1.4.3 Contrast
  (Minimum) (Level AA)`_ checks using certain themes such as dark mode. These
  are third-party, open source code bases, and so are beyond the scope of this
