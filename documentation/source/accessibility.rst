...
- The use of layout tables are due to the use of the `sphinx.ext.autosummary`_, and
  `sphinx.ext.autodoc`_ extensions. The API documentation these extensions generate
  can produce titles that fail `WCAG 2.2 1.4.3 Contrast (Minimum) (Level AA)`_
  checks using certain themes such as dark mode. These are third-party, open
  source code bases, and so are beyond the scope of this project to fix,
  although we will apply updates as this codebase develops.
- The pages generated by Sphinx contain some duplicate and empty form labels in
  ...
  Headings and Labels`_ and `WCAG 3.3.2 Labels or Instructions`_. This is a
  third-party, open source code base, and so is beyond the scope of this project
  to fix, although we will apply updates as this codebase develops.
...
Tests were carried out by the LFRic core developers. They aim to use the `WAVE`_
Web Accessibility Evaluation tool, the `lighthouse`_ tool, `axe DevTools on
Chrome`_ tool, and the built-in Chrome and Firefox accessibility dev-tools.

What we're doing to improve accessibility
-----------------------------------------

We always consider accessibility when writing documentation and aim to re-write
inaccessible, complex text as we continue development. This website is audited
for accessibility at least yearly to highlight potential accessibility issues.
