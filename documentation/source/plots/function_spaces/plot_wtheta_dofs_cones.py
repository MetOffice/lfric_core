...
With cones to make a rhombus?
"""

import plotly.graph_objects as go
import numpy as np

# sphinx_gallery_thumbnail_path = '_static/fs_thumbnails/wtheta_k0_cones.svg'
...
for i in range(len(dof_x)):
    ...
    if dof_x[i] == dof_y[i]:
        # z
        w = 1
        bottom_cone_tip_z = dof_z[i] - CONE_LENGTH
        top_cone_tip_z = dof_z[i] + CONE_LENGTH
    if dof_x[i] == dof_z[i]:
        # y
        v = 1
        bottom_cone_tip_y = dof_y[i] - CONE_LENGTH
        top_cone_tip_y = dof_y[i] + CONE_LENGTH
    if dof_z[i] == dof_y[i]:
        # x
        u = 1
...
# fig.write_image("source/_static/fs_thumbnails/wtheta_k0_cones.svg")  # for the thumbnail
