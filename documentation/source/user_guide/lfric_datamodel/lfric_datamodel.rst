...
The LFRic Architecture
======================

This section describes the :ref:`PSyKAl <psykal overview>` architecture which
governs how core scientific codes should be written for LFRic applications. The
PSyKAl architecture inspired the design of LFRic fields, and the design of the
`PSyclone <https://psyclone.readthedocs.io/en/stable/index.html>`_ tool which is
used to generate some of the application code.

PSyKAl describes a way of achieving a separation of concerns between scientific
code and technical code. A benefit of such separation is that changes to
scientific code are limited when the system running the model changes
significantly. PSyclone automatically generates the technical code to deliver
portable performance, which is a second important benefit. Portable performance
refers to the ability to more easily support and tuned the same science
applications on different compute architectures. To illustrate, PSyclone can
generate code with or without OpenMP directives, and work is underway to
generate code that will run parts of an application on GPUs.

The section begins with illustrative examples of LFRic fields that are at the
core of the LFRic data model. It then moves on to more complex examples of
PSyKAl including examples based on real code from LFRic applications and real
code generated by PSyclone. The section will not describe all of the LFRic data
model or all of PSyclone's capabilities. Subsequent sections describe other
components of the LFRic data model, and there is separate comprehensive
...
interacts mainly with the core part of this data model: the model fields.
...
grid. The new LFRic atmosphere model uses the GungHo formulation for its
dynamical core. GungHo introduces a mixed finite element scheme: fields are
represented by a combination of data points on a mesh and a set of functions
...
that encapsulates all these aspects of the field, and others.
