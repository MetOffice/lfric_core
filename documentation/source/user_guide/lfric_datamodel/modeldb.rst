...
The configuration item within the modeldb object stores the input namelists (from a
namelist input file) used to configure an instance of modeldb. Once the
configuration has been populated from file, the configuration values are
immutable.


Initialising the configuration
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The configration item is a :ref:`namelist collection type<namelist
collection>` and is populated using a module generated by the
`configurator` tool. A namelist input file is simply read in and
...
allows for different namelists to have member variables with the same
name.
...
All namelists in the collection must be unique, unless the namelist metadata
specfies `duplicate=.true.`. For namelists which may appear multiple times in
a namelist file, the `profile_name` must also be specified.
