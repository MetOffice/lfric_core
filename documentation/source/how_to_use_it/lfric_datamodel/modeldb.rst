...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
The ``config`` item is populated using a module generated by the `configurator`
tool. A namelist input file is simply read in and any valid namelists are added
to the ``config`` item.
