...
!!$  integer(i_def) :: tile_size_x, tile_size_y
...
! Remove to outside init_mesh (only required it application uses multigrid)
!!$  ! 3.0 Set up tiling
!!$  !===============================================
!!$  ! Note: There should be no reference to multigrid in this code.
!!$  !       MultiGrif is a gungho science implementation.
!!$  !
!!$  l_multigrid = .false.
!!$  if (config%namelist_exists('multigrid')) then
...
!!$
!!$    if (l_multigrid .and. (size(chain_mesh_tags) > 1)) then
