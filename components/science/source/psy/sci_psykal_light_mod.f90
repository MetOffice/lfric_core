...
contains
  ...
  !-----------------------------------------------------------------------------
  !> @brief Sets the halos of a field as clean up to some depth
  !> @details This PSyKAl-lite code is required to allow an interface to the
  !!          field's underlying halo exchange infrastructure. Accessing a
  !!          field's proxy type directly from an algorithm is not allowed in
  !!          the API, so this subroutine provides an acceptable way to do this.
  !!          This particular routine is required as a mechanism to set the
  !!          face selector fields.
  !!          NOTE: there is not an associated PSyclone issue for this, as this
  !!          does not relate to a kernel or its metadata.
  !> @param[in,out] field  The field to set clean
  !> @param[in]     depth  Depth to set the field clean to
  subroutine invoke_int_set_clean(field, depth)
  ...
  end subroutine invoke_int_set_clean
...
&undf_adspc4_face_selector_ew, map_adspc4_face_selector_ew(:,cell))
  ...
  ! -------------------------------------------------------------------------- !
  ! Face selector routines
  ! -------------------------------------------------------------------------- !
  ! These kernels have psykal-lite implementations because:
  ! (a) they must not be threaded
  ! (b) the face_counter field should not be marked as dirty, and should not be
  !     halo exchanged, to ensure that the face selectors get the correct values
  subroutine invoke_face_selector_kernel_type(face_selector_ew, face_selector_ns, face_counter)
    use mesh_mod, only : mesh_type
    use sci_face_selector_kernel_mod, only : face_selector_code
    type(integer_field_type), intent(in) :: face_selector_ew
    type(integer_field_type), intent(in) :: face_selector_ns
    type(integer_field_type), intent(in) :: face_counter
    integer(kind=i_def) :: cell
    type(mesh_type), pointer :: mesh => null()
    integer(kind=i_def) :: max_halo_depth_mesh
    integer(kind=i_def), pointer, dimension(:) :: face_selector_ew_data => null()
    integer(kind=i_def), pointer, dimension(:) :: face_selector_ns_data => null()
    integer(kind=i_def), pointer, dimension(:) :: face_counter_data => null()
    integer(kind=i_def) :: nlayers_face_selector_ew
    ...
    type(integer_field_proxy_type) :: face_selector_ew_proxy
    type(integer_field_proxy_type) :: face_selector_ns_proxy
    type(integer_field_proxy_type) :: face_counter_proxy
    ...
    face_selector_ns_proxy = face_selector_ns%get_proxy()
    face_selector_ns_data => face_selector_ns_proxy%data
    face_counter_proxy = face_counter%get_proxy()
    face_counter_data => face_counter_proxy%data

    ! Initialise number of layers
    nlayers_face_selector_ew = face_selector_ew_proxy%vspace%get_nlayers()

    ! Create a mesh object
    mesh => face_selector_ew_proxy%vspace%get_mesh()
    max_halo_depth_mesh = mesh%get_halo_depth()

    ! Look-up dofmaps for each function space
    map_w3 => face_selector_ew_proxy%vspace%get_whole_dofmap()
    map_w2h => face_counter_proxy%vspace%get_whole_dofmap()

    ! Initialise number of DoFs for w3
    ndf_w3 = face_selector_ew_proxy%vspace%get_ndf()
    undf_w3 = face_selector_ew_proxy%vspace%get_undf()

    ! Initialise number of DoFs for w2h
    ndf_w2h = face_counter_proxy%vspace%get_ndf()
    undf_w2h = face_counter_proxy%vspace%get_undf()
    ...
    ! Call kernels and communication routines
    do cell = loop0_start, loop0_stop, 1
      call face_selector_code(nlayers_face_selector_ew, face_selector_ew_data, face_selector_ns_data, face_counter_data, ndf_w3, &
  &undf_w3, map_w3(:,cell), ndf_w2h, undf_w2h, map_w2h(:,cell))
    end do

  end subroutine invoke_face_selector_kernel_type

  subroutine invoke_face_selector_halo_kernel_type(face_selector_ew, face_selector_ns, face_counter, depth)
    use mesh_mod, only : mesh_type
    use sci_face_selector_halo_kernel_mod, only : face_selector_halo_code
    type(integer_field_type), intent(in) :: face_selector_ew
    type(integer_field_type), intent(in) :: face_selector_ns
    type(integer_field_type), intent(in) :: face_counter
    integer(kind=i_def), intent(in) :: depth
    ...
    ! Call kernels and communication routines
    do cell = loop0_start, loop0_stop, 1
      call face_selector_halo_code(nlayers_face_selector_ew, face_selector_ew_data, face_selector_ns_data, face_counter_data, &
  &ndf_w3, undf_w3, map_w3(:,cell), ndf_w2h, undf_w2h, map_w2h(:,cell))
    enddo

    ! Set halos dirty/clean for fields modified in the above loop(s)
    call face_selector_ew_proxy%set_dirty()
    call face_selector_ew_proxy%set_clean(depth)
    call face_selector_ns_proxy%set_dirty()
    call face_selector_ns_proxy%set_clean(depth)

  end subroutine invoke_face_selector_halo_kernel_type
